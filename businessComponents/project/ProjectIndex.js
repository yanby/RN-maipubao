...
export default class ProjectIndex extends ListBasicComponent {
  constructor(props){
    super(props);
    this.state={
      modalObject:{},
      ...
      baseUrl: apis.projectList + '?status=开启&specialType=一手项目',
      remoteUrl: apis.projectList + '?status=开启&sort=sort,desc&specialType=一手项目' + (this.props.keywords ? ('&keywords=' + this.props.keywords) : ''),
  ...
  _menuChange(selection,row){
    let paramsString = ''
    if(selection == 0) {
      UMengAnalytics.onEvent('project_list_tab_zone_click');
    } else if(selection == 1) {
  ...
  render() {
    return (
      <View style={styles.container}>
            <StatusBar backgroundColor="transparent"  barStyle="dark-content" translucent={true}/>
            <Title isLeftShow={true} backColor={true} isRightSearchShow={true} rightChoose={true} title={'项目列表'}  onClick={() => {this.searchContent()}} >
            </Title>
            ...
            <View style={styles.dropdownView}>
                  <DropDownMenu style={styles.downView} showMoreBtn={true} doubleList={[2]}
                                ...
                                <NiceList noDataInfo={'项目添加中，敬请期待'} isNeedLoading={true} renderItem={(rowData)=><AttentionListItem {...rowData} source={'projectIndex'} />} contentContainerStyle={styles.list} remoteUrl={this.state.remoteUrl} storkey={this.state.storkey}/>
                  </DropDownMenu>
